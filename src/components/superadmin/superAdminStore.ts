...
function calcDashboardStats(tenants: SuperAdminTenant[], subs: Subscription[]): DashboardStats {
  ...
  return {
    ...
    mrr: Math.round(mrr),
    mrrChangePercent: parseFloat((mrr > 0 ? ((mrr - 22000) / 22000) * 100 : 0).toFixed(1)),
...
export const useSuperAdminStore = create<SuperAdminStore>((set, get) => ({
  ...
  refunds: [],
  payoutSummary: {
    totalRevenue: 0, totalFees: 0, totalRefunds: 0, netRevenue: 0,
    pendingPayout: 0, lastPayoutAmount: 0, lastPayoutDate: '', pendingTransactions: 0,
  },
  adminUsers: [],
  adminRoles: [],
  apiKeys: [],
  ...
  // ─── Derived ───────────────────────────────────────────
  dashboardStats: {
    totalTenants: 0, totalTenantsChange: 0, activeSubscriptions: 0,
    activeSubscriptionsChange: 0, mrr: 0, mrrChangePercent: 0,
    churnRate: 0, churnRateChange: 0, apiCallsToday: '0', apiCallsChangePercent: 0,
  },
  revenueData: [],
